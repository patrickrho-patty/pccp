...
func TestPolicyDigestIsContentAddressed(t *testing.T) {
 ...
 if d1 != d2 { t.Fatalf("digest must be stable for identical rule sets: %s vs %s", d1, d2) }
 ...
 if d3 == d1 { t.Fatal("digest must change when a rule is added") }
 // disabled rules do not count
 g.Model(&models.PolicyRule{}).Where("name = ?", "r1").Update("enabled", false)
 if s.computePolicyDigest("org-1", "models") != d1 { t.Fatal("disabling the only rule must return the empty-set digest") }
}
