...
// VersionMajor is the PAPER protocol major version (PAPER §9).
...
// MaxHeaderLen is the maximum CBOR header length (PAPER §9).
const MaxHeaderLen = 16 * 1024

// MaxPayloadLen is the maximum single DATA payload length (PAPER §9).
...
func EncodeRecord(w io.Writer, r *Record) error {
	...
	if len(r.Header) > MaxHeaderLen {
		return fmt.Errorf("paper: header exceeds max length %d", MaxHeaderLen)
	}
...
func DecodeRecord(r io.Reader) (*Record, error) {
	...
	if reserved != 0 {
		return nil, errors.New("paper: non-zero reserved field")
	}
	if rec.VersionMajor != VersionMajor {
		return nil, fmt.Errorf("paper: unsupported version %d", rec.VersionMajor)
	}
	if headerLen > MaxHeaderLen {
		return nil, fmt.Errorf("paper: header length %d exceeds max %d", headerLen, MaxHeaderLen)
	}
	if payloadLen > MaxPayloadLen {
		return nil, fmt.Errorf("paper: payload length %d exceeds max %d", payloadLen, MaxPayloadLen)
	}
