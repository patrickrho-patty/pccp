...
// Baseline interoperable hash per PAPER §37.1 is SHA-256.
const HashProfile = "PAPER-BASE-1"

// ObjectType identifies a registered PAPER object type for domain-separated
// content addressing (PAPER §32).
...
// PAPER object per §32:
//
//	digest = HASH("PAPER-OBJ-v1\0" || uint16(T) || deterministic_cbor(O))
//
...
func ComputeChunkDigest(exchangeID, laneID, laneSeq, payload []byte) Digest {
	h := sha256.New()
	h.Write([]byte("PAPER-CHUNK-v1\x00"))
	h.Write(exchangeID)
	h.Write(laneID)
	h.Write(laneSeq)
	h.Write(payload)
...
// Evidence chain domain separation strings (PAPER §34).
var (
	evidenceStartPrefix = []byte("PAPER-EVIDENCE-START-v1\x00")
	evidenceEventPrefix = []byte("PAPER-EVIDENCE-EVENT-v1\x00")
)
...
// AuthContext computes the PAPER authentication context hash (§18.2).
//
//	auth_context = HASH(
//	  "PAPER-AUTH-v1" || canonical(HELLO) || canonical(HELLO_ACK) ||
