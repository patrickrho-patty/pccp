...
// Conn wraps a TLS connection with PAPER framing.
...
// AcceptTCP wraps an accepted TLS connection with PAPER framing.
func AcceptTCP(conn net.Conn, config TransportConfig) (*TransportConn, error) {
	...
	// Read and verify PAPER preface
	if err := tc.recvPreface(); err != nil {
		return nil, fmt.Errorf("paper: recv preface: %w", err)
	}
...
func (tc *TransportConn) recvPreface() error {
	...
	for i, b := range PAPERPreface {
		if preface[i] != b {
			return fmt.Errorf("paper: invalid preface byte %d: got %02x, expected %02x", i, preface[i], b)
		}
	}
	return nil
}

// SendRecord writes a PAPER record to the connection.
...
// RecvRecord reads a PAPER record from the connection.
...
// ListenTCP creates a TLS listener for PAPER connections.
func ListenTCP(addr string, tlsConfig *tls.Config) (net.Listener, error) {
	if tlsConfig == nil {
		return nil, errors.New("paper: TLS config required for listener")
	}
	if tlsConfig.NextProtos == nil {
		tlsConfig.NextProtos = []string{ALPNProtocol}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("paper: listen %s: %w", addr, err)
	}
	return ln, nil
}

// Handshake performs the PAPER HELLO/HELLO_ACK exchange.
...
func (tc *TransportConn) Handshake(hello *HelloMessage) (*HelloAckMessage, error) {
	...
	if err := tc.SendControl(MsgHello, nil, helloBytes); err != nil {
		return nil, fmt.Errorf("paper: send HELLO: %w", err)
	}

	// Receive HELLO_ACK
	rec, err := tc.RecvRecord()
	if err != nil {
		return nil, fmt.Errorf("paper: recv HELLO_ACK: %w", err)
	}
	if rec.Kind != KindControl || MessageType(rec.MessageType) != MsgHelloAck {
		return nil, fmt.Errorf("paper: expected HELLO_ACK, got %s/%s", rec.Kind, MessageType(rec.MessageType))
	}

	var ack HelloAckMessage
	if err := UnmarshalCBOR(rec.Payload, &ack); err != nil {
		return nil, fmt.Errorf("paper: decode HELLO_ACK: %w", err)
	}
...
func (tc *TransportConn) AcceptHandshake() (*HelloMessage, error) {
	rec, err := tc.RecvRecord()
	if err != nil {
		return nil, fmt.Errorf("paper: recv HELLO: %w", err)
	}
	if rec.Kind != KindControl || MessageType(rec.MessageType) != MsgHello {
		return nil, fmt.Errorf("paper: expected HELLO, got %s/%s", rec.Kind, MessageType(rec.MessageType))
	}

	var hello HelloMessage
	if err := UnmarshalCBOR(rec.Payload, &hello); err != nil {
		return nil, fmt.Errorf("paper: decode HELLO: %w", err)
	}
...
func (tc *TransportConn) SendHelloAck(ack *HelloAckMessage) error {
	...
	if err != nil {
		return fmt.Errorf("paper: marshal HELLO_ACK: %w", err)
	}
