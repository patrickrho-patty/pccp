...
// MessageType is a PAPER message type registry value (PAPER §13).
...
// PeerProfile identifies the role of a PAPER peer (PAPER §6).
...
// HeaderKey is a numeric label for PAPER common header fields (PAPER §12).
type HeaderKey int
...
// ProtectionProfile is a PAPER payload protection level (PAPER §36).
...
// ExchangeState is the state of a governed exchange (PAPER §25).
...
// VerdictResult is the outcome of a relay enforcement verdict (PAPER §27).
...
// ConnectionState is the PAPER connection state machine (PAPER §14).
