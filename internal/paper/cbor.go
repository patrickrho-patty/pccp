...
// cborEncoderMode is the deterministic encoding mode for PAPER objects.
// Per PAPER §10.1, objects that are hashed, signed, or content-addressed MUST
...
func init() {
	...
	if err != nil {
		panic(fmt.Sprintf("paper: init cbor encoder mode: %v", err))
	}
	defaultEncMode = mode
}
