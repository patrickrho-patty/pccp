...
// HelloAckMessage is the HELLO_ACK from the Relay (PAPER §15.2).
...
// UserBindMessage binds a user identity to the connection (PAPER §19).
...
// UserBindAckMessage acknowledges a user binding (PAPER §19).
...
// CapabilitiesMessage advertises supported extensions (PAPER §20).
type CapabilitiesMessage struct {
	Extensions []string `cbor:"1,keyasint"`
}

// SessionOpenMessage requests a working session (PAPER §21).
...
// SessionGrantMessage grants a working session (PAPER §21).
...
// GovernanceEnvelope carries policy context for a governed exchange (PAPER §26).
...
// RelayVerdictMessage carries a structured relay verdict (PAPER §27).
...
// ProvenanceNodeMessage carries a provenance spine node (PAPER §33).
...
// EvidenceReceiptMessage carries an evidence receipt (PAPER §34).
...
// AIOpenMessage requests an AI inference exchange (PAPER §39.1).
...
// AICompleteMessage signals inference completion (PAPER §39.5).
