...
// Conn represents the state of a DARI connection.
type Conn struct {
	mu    sync.RWMutex
...
// Transition moves to a new state if the transition is valid (DARI §14).
func (c *Conn) Transition(newState ConnectionState) error {
	...
	if !ok {
		return fmt.Errorf("dari: no transitions from %s", c.state)
	}
	...
	return fmt.Errorf("dari: invalid transition %s → %s", c.state, newState)
}
...
var ErrNotReady = errors.New("dari: connection not in READY state")
