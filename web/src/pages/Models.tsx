...
export default function Models() {
  ...
  const [form, setForm] = useState({
    package_id: '', model_id: '', name: '', name_ko: '', family: 'code', version: '1.0.0',
  })

  const load = () => {
    fetch('/api/models', { headers: authHeaders() })
      .then(r => r.json()).then(data => setModels(Array.isArray(data) ? data : []))
      .catch(() => setModels([]))
  }
  useEffect(() => { load() }, [])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/models', {
        ...
        body: JSON.stringify({ ...form, state: 'draft' }),
      })
      if (!res.ok) throw new Error('등록 실패')
      setShowForm(false)
      setForm({ package_id: '', model_id: '', name: '', name_ko: '', family: 'code', version: '1.0.0' })
      load()
  ...
  const handlePublish = async (id: string) => {
    try {
      await fetch(`/api/models/${id}/publish`, { method: 'POST', headers: authHeaders() })
      load()
    } catch {}
  }

  const handleRecall = async (id: string) => {
    if (!confirm('이 모델을 리콜하시겠습니까? 모든 엔드포인트 리스가 무효화됩니다.')) return
    try {
      await fetch(`/api/models/${id}/recall`, { method: 'POST', headers: authHeaders() })
      load()
    } catch {}
  }
  ...
  const handleEdit = (m: any) => {
    setEditingId(m.id)
    setForm({
      package_id: m.package_id || '', model_id: m.model_id || '',
      name: m.name || '', name_ko: m.name_ko || '', family: m.family || 'code',
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(`/api/models/${editingId}`, {
        ...
        body: JSON.stringify({ name: form.name, name_ko: form.name_ko }),
      })
      setEditingId(null)
      setShowForm(false)
      load()
  ...
  const stateLabel = (s: string) => {
    const map: Record<string, string> = { draft: '초안', published: '게시됨', deprecated: '사용중단', recalled: '리콜됨' }
    return map[s] || s
  }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => {
          if (editingId) { setEditingId(null); setForm({ package_id: '', model_id: '', name: '', name_ko: '', family: 'code', version: '1.0.0' }) }
        ...
        <form onSubmit={editingId ? handleUpdate : handleCreate} className="card mb-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {!editingId && (
              <>
                <div>
                  <label className="label">패키지 ID · Package ID</label>
                  <input className="input font-mono text-xs" value={form.package_id} onChange={e => setForm({ ...form, package_id: e.target.value })} placeholder="pmp_qwen3_moe_v1" required />
                </div>
                <div>
                  <label className="label">모델 ID · Model ID</label>
                  <input className="input" value={form.model_id} onChange={e => setForm({ ...form, model_id: e.target.value })} placeholder="qwen3-moe" required />
                </div>
              </>
            )}
            <div>
            ...
            <div>
              ...
              <input className="input" value={form.name_ko} onChange={e => setForm({ ...form, name_ko: e.target.value })} placeholder="큐웰3 MoE" />
            </div>
            {!editingId && (
              <>
                <div>
                  <label className="label">패밀리 · Family</label>
                ...
                <div>
                  <label className="label">버전 · Version</label>
      ...
      <div className="card">
        {models.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-400">등록된 모델이 없습니다</p>
          </div>
        ) : (
          <table className="w-full">
            ...
            <tbody>
              {models.map(m => (
                <>
                  <tr key={m.id} className="border-b border-gray-100 last:border-0 hover:bg-blue-50/30 cursor-pointer"
                    ...
                    <tr className="bg-gray-50">
                      <td colSpan={6} className="p-4">
                        <div className="grid grid-cols-3 gap-4 text-sm">
                          ...
                          <div><span className="text-gray-500">매니페스트 다이제스트:</span> <code className="text-xs">{m.manifest_digest?.slice(0, 30) || '-'}</code></div>
                          <div><span className="text-gray-500">서명 키 ID:</span> {m.signature_key_id?.slice(0, 20) || '-'}</div>
                          ...
                          <div className="col-span-3">
                            ...
                            <code className="text-xs bg-white px-1.5 py-0.5 rounded border border-gray-200">{m.signature?.slice(0, 40) || '-'}...</code>
                          </div>
                        </div>
                      </td>
                    </tr>
