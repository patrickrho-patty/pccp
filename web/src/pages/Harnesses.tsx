...
export default function Harnesses() {
  ...
  const handleEnroll = async (e: React.FormEvent) => {
    ...
    try {
      await api.enrollHarness({
        ...form, organization_id: orgId, enrollment_mode: 'sso',
        device_hostname: 'dev-machine', device_os: 'darwin', device_arch: 'arm64',
      })
      setShowForm(false)
      setForm({ user_id: '', harness_id: '', public_key_hex: '', binary_version: '1.0.0' })
      load()
  ...
  const handleRevoke = async (id: string) => {
    ...
    try { await api.revokeHarness(id, 'manual revoke'); load() } catch {}
  }

  const handleQuarantine = async (id: string) => {
    if (!confirm('이 하네스를 격리하시겠습니까? 모든 활성 세션이 종료됩니다.')) return
    try { await api.quarantineHarness(id); load() } catch {}
  }

  const handleReactivate = async (id: string) => {
    try { await api.reactivateHarness(id); load() } catch {}
  }

  const getUserName = (harness: any) => {
    if (!users.length) return '-'
    const allowedUsers = harness.allowed_users
    return harness.harness_id || '-'
  }

  const statusBadge = (s: string) => {
    const map: Record<string, string> = {
      enrolled: 'badge-green', active: 'badge-green', pending: 'badge-yellow',
      quarantined: 'badge-red', revoked: 'badge-gray',
    }
  ...
  const statusLabel = (s: string) => {
    const map: Record<string, string> = {
      enrolled: '등록됨', active: '활성', pending: '대기',
      quarantined: '격리됨', revoked: '폐기됨',
    }
    return map[s] || s
  }
        ...
        <form onSubmit={handleEnroll} className="card mb-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="input font-mono text-xs" value={form.harness_id} onChange={e => setForm({ ...form, harness_id: e.target.value })} placeholder="hrn_xxx" required />
            </div>
            <div>
            ...
            <div>
              <label className="label">버전 · Binary Version</label>
              <input className="input" value={form.binary_version} onChange={e => setForm({ ...form, binary_version: e.target.value })} />
            </div>
          </div>
        ...
        {harnesses.length === 0 ? (
          ...
          <table className="w-full">
            ...
            <tbody>
              {harnesses.map((h) => (
                <>
                  <tr key={h.id} className="border-b border-gray-100 last:border-0 hover:bg-blue-50/30 cursor-pointer"
                    ...
                    <td className="py-3">
                      <span className={h.risk_state === 'normal' ? 'badge-green' : h.risk_state === 'high' ? 'badge-red' : 'badge-yellow'}>
                        {h.risk_state}
                      </span>
                    </td>
                    ...
                    <td className="py-3" onClick={e => e.stopPropagation()}>
                      <div className="flex gap-2 justify-end">
                        {(h.status === 'enrolled' || h.status === 'active') && (
                          <>
                            <button onClick={() => handleQuarantine(h.id)} className="text-yellow-600 text-xs hover:underline">격리</button>
                            <button onClick={() => handleRevoke(h.id)} className="text-red-600 text-xs hover:underline">폐기</button>
                          </>
                        )}
                        {h.status === 'quarantined' && (
                          <button onClick={() => handleReactivate(h.id)} className="text-green-600 text-xs hover:underline">재활성화</button>
                        )}
                      </div>
                    </td>
                  </tr>
                      ...
                      <td colSpan={6} className="p-4">
                        <div className="grid grid-cols-3 gap-4 text-sm">
                          ...
                          <div className="col-span-3">
                            ...
                            <code className="text-xs bg-white px-1.5 py-0.5 rounded border border-gray-200">{h.public_key?.slice(0, 40)}...</code>
                          </div>
                          {h.revocation_reason && (
                            <div className="col-span-3 text-red-600">폐기 사유: {h.revocation_reason}</div>
                          )}
                        </div>
                      </td>
                    </tr>
