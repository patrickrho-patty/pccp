// Enterprise harness feature catalog — immutable reference data shared by
// the page UI, the change validator, and the changeset recorder. Keys here
// must match the seed defaults in internal/api/server.go
// (handleSeedEnterpriseFeatures) and the mandatory-feature set on the
// server (pattyMandatoryEnterpriseFeatures).
...
// Re-export so the catalog file remains the single import surface for
// catalog consumers that also need the heartbeat-staleness predicate.
export { isHarnessOnlineNow }
