# DARI Protocol — Open Source Repository

> **Patty AI Provenance & Enforcement Relay**
>
> DARI is an open communication protocol for governed AI inference. It connects AI coding agents to model endpoints through a verifiable, policy-enforced, provenance-tracked data path.

## Quick Start

### Build a PIA (Patty Inference Agent)

A PIA bridges any model serving engine (vLLM, SGLang, TGI) to the DARI protocol.
...
func main() {
    ...
    pia.Listen(":9444") // DARI protocol listener
}
```
...
```
Harness ←──DARI──→ Relay ←──DARI──→ PIA ←──local──→ vLLM/SGLang
```

| Concept | Description |
|---|---|
| **DARI** | Binary protocol over TLS/TCP or QUIC using CBOR records |
| **Harness** | The AI coding agent (developer's machine) |
| **Relay** | Horizontally scalable data plane with governance enforcement |
| **PIA** | Patty Inference Agent bridging DARI to local serving engines |
...
```
pccp/
├── internal/dari/          # DARI protocol library (Go)
...
│   ├── client.go            # Reference DARI client
│   ├── ai_v2.go             # DARI AI Semantic v2 (tools, multimodal, streaming)
│   ├── models.go            # dari.models/1 catalog extension
│   └── handshake.go         # All DARI message types (CBOR)
├── adapters/
...
├── registry/
│   ├── messages.csv         # DARI message type registry
│   ├── profiles.csv         # Peer profiles
│   ├── errors.csv           # Error code families
│   └── crypto.csv           # Cryptographic profiles
├── conformance/             # Conformance test suite
└── docs/plans/DARI/        # Full protocol specification
```

## Building a Custom Adapter

Any inference engine can be connected to DARI by implementing the `EngineAdapter` interface:
...
1. **DARI is the sole Harness protocol** — no OpenAI/Anthropic HTTP fallback
