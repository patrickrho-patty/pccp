...
>
> Grounded in: `internal/provider/paper/paper.go` (AI_OPEN → single AI_COMPLETE; connector already decodes `AI_TOKEN_CHUNK`), `pccp/internal/relay/paper_listener.go governAIOpen` (forwards inference, emits one AI_COMPLETE — **no token streaming on the governed path**), `internal/paper/messages.go` (AI_TOKEN_CHUNK 0x0402 defined, unused relay-side), and the comparison target `~/projects/codex/codex-rs/core/src/client.rs` (Responses API over a **cached, prewarmed WebSocket** — lazy open, reused across turns, `response.create` with `generate=false` prewarm — with SSE/eventsource HTTP fallback).

## What exists (honest baseline)
- PAPER transport is a **persistent TLS 1.3 connection with binary CBOR framing** — no per-request HTTP upgrade, no SSE text parsing, no JSON-over-headers. Connection reuse exists (`connect()` pings and reuses).
...
- Codex prewarms its WebSocket between turns. *Fix:* connector prewarms the PAPER connection + session (SESSION_OPEN at idle end of turn, lease acquire) so turn-start cost is ~zero; measure with/without.
