...
### E2. Comms hub delivered in-IDE over PAPER (§21.2/§22.4)
- Comms is CP-side only (web 13); the harness never receives chat/presence/broadcast/admin-commands. *Fix:* a PAPER comms channel to enrolled harnesses; admin commands execute + emit audit.
...
- PCCP fleet actions change DB but never reach the live harness. *Fix:* governed admin commands (lock/freeze-branch/force-update/recall) delivered to the harness, executed, evidence emitted (ties to Harness A PAPER records + B evidence).
