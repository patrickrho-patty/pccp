...
The immediate answer is no: none of the measured primitives is an obvious showstopper. Record encode/decode scales approximately linearly across the measured sizes, reaching 97.0~$\mu$s for a 1~MiB payload. The 73.05~ns evidence-chain update is small enough to apply per evidence event, and a 42.7~$\mu$s sign-and-verify operation is plausible at exchange or receipt boundaries rather than per token. The benchmark allocates an encoded buffer and decoded record on every iteration; bytes allocated are therefore roughly twice the large payload size and identify a concrete optimization target.
...
Generated conceptual artwork is used only for Figure~\ref{fig:overview}; it communicates architecture and contains no measurement or factual comparison. Figures~\ref{fig:lifecycle} and~\ref{fig:model-identity} are LaTeX-native protocol-design diagrams. Figure~\ref{fig:primitive-latency} and the empirical tables are derived from the checked-in benchmark dataset; raw per-run output remains a requirement for a complete empirical release.
