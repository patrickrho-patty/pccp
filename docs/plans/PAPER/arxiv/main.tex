...
DARI is published as an open specification whose evidence status is defined in the checked-in \texttt{implementation-claim-matrix.tsv}. The benchmark snapshot is repository commit \texttt{0234034}. That implementation establishes the record layer and transport bindings measured below; it does not by itself establish the stronger authentication, authorization, delegation, or receipt claims.
...
C3 & Transcript-bound peer authentication & implementation-required & \texttt{internal/relay/peer\_authenticator\_test.go} \\
C4 & Canonical signed authorization scope & implementation-required & \texttt{internal/paper/authorization\_test.go} \\
C5 & Delegated authorization attenuation & implementation-required & \texttt{internal/paper/delegation\_test.go} \\
C6 & Deterministic final receipt & implementation-required & \texttt{internal/paper/receipt\_test.go} \\
