...
**Protocol name:** **DARI — Patty AI Provenance & Enforcement Relay**  
...
**Architecture decision:** All governed AI traffic traverses a horizontally scalable DARI Relay data plane controlled by Patty Code Control Plane (CP)
...
DARI is Patty's open communication protocol for AI engineering systems. It is designed to connect the Patty Code Harness, Patty Code Control Plane, DARI Relay, Patty Inference Agent (PIA), runtime services, and approved GPU/model infrastructure through one stateful, authenticated, governance-native protocol.

DARI is deliberately **not** an OpenAI-compatible API with different endpoint names. It does not use `/v1/chat/completions`, `/v1/responses`, Anthropic-style `/messages`, generic HTTP POST endpoints, JSON-RPC, WebSocket, or SSE as its normal remote communication model. It uses QUIC as the preferred transport and a native TLS 1.3/TCP binding as a compatibility fallback for networks that block QUIC. Both bindings carry the same DARI application protocol.
...
Instead, DARI treats every meaningful AI operation as a **Governed Exchange**:
...
DARI is also the **complete communication substrate of the Harness**, not only the inference path. A connected enterprise Harness uses the same authenticated DARI connection for:
...
DARI therefore becomes the trusted communications fabric between the developer surface, organizational governance, and AI infrastructure.
...
```text
                  ...
                  identity / policy / registry / audit
                          / provenance / admin
                                  ...
                                  v
+----------------+       +--------------------+       +----------------+
| Patty Code     | DARI | DARI Relay        | DARI | PIA            |
...
DARI uses standard, reviewed cryptography. The project will not invent its own cipher, KEM, signature primitive, or secure transport. Novelty belongs in the **protocol semantics and security architecture**, including:
...
4. **DARI Provenance Spine** — content-addressed, cryptographically linked lineage connecting user intent, prompts, context, models, tools, generated artifacts, human edits, commits, and approvals.
...
DARI addresses these requirements by treating communication itself as an enterprise control surface.
...
DARI therefore does not claim novelty for encrypted transport, multiplexing, signatures, public-key encryption, or group key establishment. These are mature areas and should use existing standards.

DARI's product novelty is the composition and semantics of a communication protocol specifically designed for governed AI engineering.

## 3.2 What DARI intentionally reuses

DARI should reuse established standards where appropriate:
...
## 3.3 What must be genuinely DARI-specific

DARI must introduce its own coherent model for:
...
The open-source repository must clearly identify which mechanisms are DARI inventions and which are standards reused for safety/interoperability.
...
| Data plane | DARI Relay, horizontally scalable |
...
| Collaboration | Built into DARI |
| Voice messages | Built into DARI |
| File transfer | Built into DARI and governed |
| Broadcasts | Built into DARI |
...
## G3 — Restrict enterprise Harness communication to the DARI network

The official Patty Code enterprise Harness should communicate with organizational AI services through DARI rather than generic model APIs. A user should not be able to configure `OPENAI_BASE_URL` or an Anthropic endpoint and transparently substitute it for CP.
...
Uses Patty Code against Patty-hosted models. The public Harness still speaks DARI, but Patty operates the Relay/PIA/CP services. The individual user does not receive the enterprise administrative console.
...
| Term | Definition |
|---|---|
| **DARI** | Patty AI Provenance & Enforcement Relay protocol family. |
...
| **Relay** | Horizontally scalable DARI data-plane service that terminates/forwards DARI streams and performs inline enforcement. |
...
| **Connection** | One QUIC or TLS/TCP secure transport connection carrying DARI. |
...
DARI exists inside a larger Patty Code platform but must remain a separable open-source project.
...
```text
Enterprise Harness -> Customer DARI Relay -> Customer PIA -> Customer GPU
                       ^
                       |
                    Local CP
```
...
DARI must distinguish identities that traditional APIs often collapse.
...
Without hardware-backed remote attestation, CP cannot mathematically prove that an open-source Harness binary on a machine controlled by an attacker is byte-for-byte unmodified. DARI can strongly authenticate an **enrolled Harness identity**, detect revoked/duplicated/anomalous identities, require signed official build metadata, and bind credentials to protocol state; it cannot turn software-only self-reporting into a hardware root of trust.

This limitation must be documented rather than hidden.

Organizations requiring stronger executable integrity may enable an optional higher-assurance profile using platform attestation/MDM/TPM/Secure Enclave or equivalent controls, but this is not a general DARI requirement.

---
# 11. Protocol Layering

DARI has a layered design so transport, identity, protocol semantics, and application capabilities can evolve independently.

```text
+---------------------------------------------------------------+
| Harness / Relay / PIA / Runtime Application                   |
+---------------------------------------------------------------+
| DARI Capability Extensions                                  |
| AI | Context | Tools | Provenance | Policy | Chat | Voice ... |
+---------------------------------------------------------------+
| DARI Core                                                    |
| Peer identity | Session | Stream Contract | Exchange | Error   |
+---------------------------------------------------------------+
| DARI Secure-Channel Binding                                  |
...
The protocol must never depend on HTTP semantics. There is no URL routing model, status-code dependency, cookie model, `Authorization: Bearer`, HTTP content negotiation, or browser-origin assumption at the DARI layer.

DARI implementations may expose separate local administrative HTTP APIs for unrelated operational purposes, but those interfaces are not DARI and must not become a hidden compatibility path for the Harness.
...
QUIC is the preferred transport because DARI needs multiple concurrent streams with different lifetimes and flow-control characteristics: inference token streams, chat, large file transfers, broadcasts, control messages, and telemetry should not be serialized behind one application-level TCP stream.

Requirements:

- QUIC v1 or a later explicitly approved version.
- TLS security as defined for the selected QUIC version.
- ALPN identifying DARI, initially using a project-controlled experimental/private identifier; pursue formal registration when the protocol stabilizes.
- UDP/443 default for public/enterprise compatibility, configurable for private deployments.
- QUIC connection migration may be supported for mobile/network transitions but must not bypass DARI peer/session policy.
- 0-RTT application data is **disabled by default** for DARI v1 because replayable early data is inappropriate for policy-changing, tool, file, chat, or inference actions. Future use may be restricted to explicitly idempotent messages.
...
```text
TCP -> TLS 1.3 -> ALPN paper/* -> DARI Core
```
...
The TCP binding requires a DARI multiplexing layer because TCP provides one ordered byte stream. A blocked file transfer must not indefinitely block a control/broadcast stream.

## 12.3 Transport selection

Harness behavior:

1. Resolve configured DARI service.
...
DARI is one protocol family with authenticated peer profiles. Profiles are not cosmetic labels; the credential and protocol state machine bind the peer to allowed message families.
...
CP issues Harness Credential
        |
        v
Harness may establish DARI connections
```
...
Conceptual DARI exchange after TLS/QUIC security establishment:
...
The **Exchange** is the central application object of DARI.

## 17.1 Exchange philosophy

Traditional RPC often asks: “What method is being called?” DARI additionally asks:
...
DARI must represent more than chronological order. An exchange may explicitly state:
...
Capability Leases are a central DARI-specific security primitive at the application layer.
...
The Provenance Spine is the protocol-level causal/evidence graph that makes DARI materially different from a model API plus logs.
...
For token streaming, DARI must not require a heavyweight signature for each token. Implementations may group chunks into incremental hash chains or Merkle-style checkpoints while preserving the ability to detect omission/reordering within the evidence policy.
...
12. Relay opens downstream DARI inference stream.
...
2. PIA establishes DARI connection as `INFERENCE` profile.
...
9. PIA streams model output back over DARI.
...
A PIA running on a host fully controlled by a malicious root administrator can theoretically falsify software-only measurements. DARI baseline therefore promises authenticated PIA/model package workflow and operational enforcement, not impossible remote proof without a hardware root of trust.
...
DARI is not one giant ordered stream.
...
On QUIC, lanes map to one or more QUIC streams. On TCP, DARI multiplexes them using application stream IDs and independent flow-control/priority semantics.
...
DARI AI messages represent an agentic inference lifecycle, not merely a text completion.
...
DARI should support references/content for:
...
Retrieved content can contain instructions. DARI context objects carry trust attributes such as:
...
DARI must represent agent tool activity as governed exchanges.
...
DARI does not assume tools run on the developer host. Tool exchange can target a `RUNTIME` peer representing a local governed runner, remote sandbox, CI runner, or government microVM pool.

---

# 33. Collaboration Message Family

DARI provides contextual engineering collaboration inside the Harness.
...
Initial DARI voice support focuses on **asynchronous voice messages**, not full real-time calling.
...
Live voice/calling is not required for v1. If added, it should use a separate DARI real-time media extension rather than overloading asynchronous voice-message semantics.
...
Broadcasts use DARI but have stronger delivery and acknowledgement semantics than ordinary chat.
...
DARI can deliver control-plane actions to connected Harness/Relay/PIA peers.
...
DARI carries operational telemetry but telemetry should not compete with interactive traffic.
...
Transport encryption protects data between immediate peers. DARI additionally needs explicit semantics for **who is permitted to access plaintext**.
...
- Use a standard group key protocol such as MLS rather than inventing a DARI group-crypto algorithm.
...
Even when payload is endpoint-sealed, DARI should minimize exposed metadata and clearly document what remains visible: peer identity, sizes, timing, policy labels, routing, and exchange references may still reveal information.

---

# 41. Cryptography and Key Management

DARI does not define proprietary cryptographic primitives.
...
DARI defines algorithm identifiers/registries so deployments can meet different cryptographic requirements.
...
Agent sessions can last minutes or hours. Networks fail. DARI must resume safely without replaying side effects.
...
DARI v1 must not execute protected application actions from QUIC 0-RTT early data. Future versions may permit narrowly defined idempotent messages after explicit analysis.
...
DARI must be usable by enterprises that update slowly and government systems that may remain offline for extended periods.

## 43.1 Version layers

- transport binding version,
- DARI core major version,
...
DARI errors must be machine-actionable and human-explainable.
...
DARI's security design must be tested against explicit adversaries.
...
| Protocol reverse engineering | attacker implements DARI | safe by design; open spec; auth/policy remains boundary |
...
DARI makes the official Harness non-compatible with generic external model endpoints. Preventing a malicious employee from writing a separate HTTPS client to OpenAI requires OS/network egress controls outside the protocol. CP should integrate with enterprise network/security controls, but DARI must not overclaim this guarantee.
...
DARI supports strong administrative visibility but does not treat administrator access as inherently unregulated.
...
DARI is globally implementable but Korean enterprise behavior is a first-class product input.
...
DARI itself carries classification/result metadata; detection engines are replaceable components.
...
DARI must supply sufficient signed/structured events to power those views.
...
Korean enterprises frequently require centralized rapid control. DARI should make it possible to:
...
Government uses the **same DARI protocol**, not a separate fork.
...
Government profile supports deployment-specific allowed suites and KCMVP-aware module requirements where applicable. DARI's wire semantics remain algorithm-agile.
...
Chat/voice/file/broadcast remain fully functional inside the closed DARI deployment, which is an important differentiator versus cloud-dependent collaboration tools.
...
DARI is a protocol project, but CP is its authoritative enterprise control system.

## 51.1 CP responsibilities consumed by DARI
...
The internal distribution implementation is not necessarily DARI v1, though using an `ADMIN_AGENT`/service DARI profile is preferred if it remains clean.

## 51.3 Administrative actions

CP admin UI creates signed control objects which DARI delivers to the relevant Relay/Harness/PIA. DARI receipts return execution/acknowledgement state.

---

# 52. Git/SCM and Code Provenance Integration

DARI's engineering provenance becomes valuable only if it maps to source-control reality.
...
DARI provenance should be exportable or translatable to established software-provenance/attestation ecosystems where practical, while preserving DARI-specific AI/governance fields.

---

# 53. Billing, Metering, and Entitlements

DARI carries trusted usage signals needed by public and enterprise product models.
...
DARI implementations must be operationally transparent without leaking unnecessary payload content.
...
Internal distributed traces correlate using DARI connection/session/exchange IDs. Payload data should not automatically be copied into tracing tags.
...
- DARI Relay service availability: 99.95% per region/cluster target,
...
DARI's additional governance must be measurable and optimized.
...
DARI must be credible as an open protocol, not merely “source visible.”
...
If Patty owns the DARI trademark, define fair rules allowing conforming implementations to state compatibility without implying Patty endorsement.

---

# 58. Protocol Registry and Extension Governance

DARI should maintain machine-readable registries for:
...
When DARI stabilizes and adoption justifies it, pursue appropriate public registration such as an ALPN protocol identifier and any other relevant registries rather than assuming an unregistered value forever.
...
Adapters remain behind PIA; they are not DARI protocol endpoints exposed to Harnesses.
...
Two test peers can authenticate over QUIC and TCP, negotiate identical DARI core semantics, and verify a signed Capability Lease/channel binding.
...
Enterprise employees can collaborate entirely inside a private DARI deployment with policy-controlled attachments and emergency broadcast delivery.
...
| OpenAI-compatible bypass by separate client | data leakage | endpoint/network egress controls outside DARI |
...
The following are required before DARI 1.0:
...
4. An unregistered peer implementing the public DARI spec cannot enter an authenticated enterprise session.
...
29. QUIC 0-RTT cannot cause side-effecting DARI actions in v1.
...
DARI reaches v1.0 product readiness when:
...
7. **Collaboration:** Enterprise users can chat, send voice messages/files, see presence, and receive broadcasts inside DARI.
...
14. **Novelty clarity:** documentation distinguishes DARI's novel governance/provenance semantics from reused Internet/cryptographic standards.

---

# 66. Research and Publication Requirements

DARI is intended for open-source publication and an arXiv systems/security paper. Product development should capture experimental evidence from the start.
...
Compare DARI to representative conventional patterns, not to strawmen:
...
The paper must not claim MCP/A2A/in-toto/SLSA are inferior at goals they do not attempt. The contribution is that DARI targets a different boundary: **governed Harness-to-AI infrastructure communication with live policy and causal provenance**.
...
| `CORE_CONNECTION_BIND` | both | bind DARI identity state to secure channel |
...
Harness opens TCP/443 + TLS 1.3
  ...
  -> DARI multiplexing maps logical streams
```
...
DARI must be designed with awareness of adjacent open protocols while avoiding false claims of uniqueness.

## F.1 Conventional model APIs

OpenAI/Anthropic-style APIs are optimized around remote model interaction and streaming. DARI intentionally differs by making organization identity, policy epochs, capability leases, Relay enforcement, and causal provenance intrinsic protocol objects and by refusing generic model API compatibility at the Harness boundary.

## F.2 Model Context Protocol (MCP)

MCP is designed to connect AI applications to tools/context/resources. Its standardized remote transport uses JSON-RPC and Streamable HTTP (with stdio also defined). DARI is not intended to replace MCP tool semantics in the ecosystem. Patty Code may still use MCP **inside a governed tool model**, while DARI governs the Harness/Relay/inference and enterprise communication boundary.

DARI novelty claim must therefore not be “first protocol to let AI use tools.”

## F.3 Agent2Agent (A2A)

A2A targets interoperability between independent agent systems, including capability discovery, tasks, messages, and artifacts, with HTTP(S)/JSON-RPC-based bindings in its specification. DARI does not primarily target open agent discovery/inter-vendor task delegation. DARI targets authenticated enterprise Harness participation, inline governance, inference infrastructure, and provenance.

DARI novelty claim must therefore not be “first agent communication protocol.”

## F.4 in-toto / SLSA

in-toto and SLSA provide important software-supply-chain provenance/attestation concepts. DARI should interoperate/export where useful rather than pretending provenance is new.

The distinction is timing and boundary:

- in-toto/SLSA focus on verifiable software supply-chain steps/build artifacts,
- DARI generates causal provenance during live human-AI engineering exchanges, including prompts, context, policy verdicts, model identity, tools, human edits, and collaboration references before final build provenance exists.

DARI novelty claim must therefore not be “first signed software provenance.”

## F.5 TLS/QUIC/HPKE/MLS

These are security/transport building blocks. DARI's novelty is not their cryptographic properties.

## F.6 Candidate research contribution wording

A defensible formulation is:

> **DARI proposes a governance-native communication model for human-AI software engineering in which purpose-bound authority, inline enforcement decisions, and causal provenance are intrinsic to the lifecycle of multiplexed AI and collaboration exchanges.**
...
DARI v1 design should reference the current official specification for each reused standard at implementation/release time. As of this PRD baseline, important standards/specifications include:
...
- **RFC 9180** — Hybrid Public Key Encryption (HPKE), if DARI payload sealing uses it.
...
The formal DARI specification must use normative references only where DARI actually depends on a standard. Related-work references belong in informative sections and the research paper.

---

# Final Product Statement

DARI is successful if an enterprise or government organization can make the following statement and prove it with the protocol's own evidence:
...
That—not custom encryption or different endpoint names—is the product DARI is intended to deliver.
...
This addendum freezes the implementation acceptance contract for **DARI — Delegated Authorization and Receipts for Inference**. It is consumed by Phase 2 implementation Tasks 5–19. Normative schemas and algorithms are in Appendix F of `DARI_Protocol_Specification_v1.0.md`; profile and migration behavior is in `DARI_COMPATIBILITY_AND_PROFILE_MAP.md`. Those two documents control if older DARI requirements conflict with this addendum for a DARI profile.
...
```text
dari/1 record and message encoding -> DARI legacy compatibility profile
...
|---|---|
| Task 5 — compatibility harness | Byte-exact `dari/1` framing, signing, digest, receipt, and allocation vectors plus explicit legacy-variant behavior |
...
- legacy/DARI parser isolation and frozen `dari/1` bytes;
...
- rewrite the complete historical DARI specification or product vocabulary merely to rename it;
