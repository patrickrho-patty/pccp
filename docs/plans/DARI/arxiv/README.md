...
The PDF is written to `DARI_arXiv.pdf`.
...
```sh
go test ./internal/dari -run '^$' -bench 'Benchmark(RecordRoundTrip|CanonicalCBOR|EvidenceChainNext|ObjectDigest|Ed25519SignVerify)$' -benchmem -count=5
go test ./... -count=1
```
