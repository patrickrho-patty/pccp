...
**Reference architecture:** Harness ↔ DARI Relay ↔ Patty Inference Agent (PIA) ↔ inference engine/GPU  
...
DARI — **Patty AI Provenance & Enforcement Relay** — is an open, stateful application protocol for communication between AI coding harnesses, governance relays, inference infrastructure, controlled runtimes, and human collaboration services.

DARI is designed for a boundary that conventional model APIs, generic RPC frameworks, agent-to-agent protocols, and post-build provenance systems do not directly address: **live communication between an AI engineering client and AI infrastructure where authority, policy enforcement, causal provenance, and evidence are part of the exchange itself**.

A DARI deployment separates the administrative **Control Plane** from a horizontally scalable **DARI Relay data plane**. Harnesses do not call raw model endpoints. A Harness establishes DARI with an enrolled Relay. The Relay authenticates the Harness and user, validates a time-bounded **Capability Lease**, binds the exchange to a **Policy Epoch**, performs required security and governance controls, and forwards authorized AI traffic to an enrolled **Patty Inference Agent (PIA)**. PIA is the only component permitted to translate DARI inference messages into a local serving-engine interface such as vLLM or SGLang.

DARI also serves as the complete communication substrate of an enterprise Harness. The same protocol carries coding sessions, context manifests, tool proposals and results, provenance events, employee chat, voice messages, governed file transfers, presence, administrative notices, broadcasts, telemetry, and metering. Capabilities are separated by peer profile, extension, Stream Contract, and lease rather than by assuming all authenticated peers may use all features.

DARI deliberately does **not** define new cryptographic primitives. It composes standardized secure transport and cryptographic building blocks, including TLS 1.3, QUIC, ALPN, TLS channel binding/exporters, CBOR, COSE, and, where enabled, HPKE or MLS. DARI's protocol-specific contribution is the semantics and lifecycle of governed exchanges, capability-bound streams, inline Relay verdicts, and causal provenance.
...
A conforming DARI implementation implements one or more peer profiles defined by this specification and satisfies the mandatory behavior for every enabled extension.
...
DARI specifies communication for:

1. an official or conforming AI Harness communicating with an authorized DARI Relay;
2. a DARI Relay communicating with a registered PIA;
3. a Relay communicating with controlled runtime agents;
4. a Harness exchanging enterprise collaboration messages through DARI infrastructure;
...
DARI is not OpenAI-compatible, Anthropic-compatible, or an HTTP REST API.
...
DARI records explicit parent relationships between user intent, context, policy decisions, model actions, tool calls, file changes, reviews, and artifacts. Wall-clock ordering is supplemental evidence; it is not the sole provenance model.
...
The wire format and reference implementation may be fully public. Security MUST remain valid if an attacker knows every protocol detail. Participation in a controlled DARI deployment depends on enrolled identity and authorization.

## 3.7 Transport parity

The QUIC and TLS/TCP bindings MUST preserve the same DARI semantics. A TCP fallback MUST NOT silently weaken authentication, policy, provenance, or payload protection.
...
```text
                    ...
                    │       governed data plane          │
                    │                                    │
Harness ══ DARI ══►│ auth / lease / policy / DLP        │══ DARI ══► PIA
...
DARI Relay is the inline data-plane enforcement point. It MAY be horizontally replicated. It MAY be decomposed into connection, inspection, routing, and evidence services as long as externally observable DARI behavior remains conformant.
...
**Peer** — an authenticated DARI participant.

**Peer Profile** — a constrained role defining which message families and state transitions a peer may use.

**Connection** — one QUIC or TLS/TCP transport instance carrying DARI.

**Working Session** — a longer-lived user/Harness/project interaction that may survive transport reconnects.

**Lane** — a logical ordered DARI stream. A lane is transport-independent and may map to a QUIC stream or to a multiplexed TCP sequence.
...
**PIA** — Patty Inference Agent, an enrolled DARI INFERENCE peer that binds a serving endpoint to a PMP and executes Relay-authorized inference.
...
- MUST negotiate DARI with ALPN;
- MUST authenticate the Relay using the configured TLS trust model;
- MUST NOT send protected DARI application actions as 0-RTT early data in version 1;
- MUST apply QUIC transport flow control in addition to DARI lane limits.
...
A TCP DARI server SHOULD accept TCP/443 where practical.
...
- MUST implement DARI logical-lane multiplexing.
...
A peer MUST NOT fall back from DARI to an OpenAI-compatible, Anthropic-compatible, or other generic model protocol.

## 7.4 Channel binding

DARI authentication proofs are bound to the secure transport with exported keying material compatible with TLS 1.3 channel binding.

DARI uses the TLS 1.3 `tls-exporter` channel-binding value defined by RFC 9266. The value is 32 bytes of Exported Keying Material obtained with the exact RFC 9266 inputs:
...
DARI includes this `tls-exporter` value in the higher-level peer-authentication context together with the DARI HELLO/HELLO_ACK transcript and nonces. DARI does not redefine the channel-binding derivation or introduce a private exporter label.
...
Migration does not create a new DARI Working Session, but the Relay SHOULD generate a network-path-change telemetry event.
...
After QUIC establishment and ALPN negotiation, the client opens the first client-initiated bidirectional stream as the DARI Control Lane and sends `HELLO` as its first DARI record.

## 8.2 TLS/TCP

Immediately after TLS and ALPN negotiation, each side expects the following eight-octet connection preface before ordinary DARI records:
...
DARI uses a fixed 32-octet binary prelude followed by a deterministic-CBOR header and optional payload bytes.
...
| 3 | `ACK` | Explicit DARI-level acknowledgement |
| 4 | `RESET` | Lane or operation reset |
| 5 | `RECEIPT` | Evidence/provenance receipt |
| 6 | `ERROR` | Structured error |
| 7 | `PING` | DARI-level liveness/latency probe |

Transport integrity protects the record bytes. DARI does not add an unauthenticated CRC.
...
Structured DARI objects use CBOR.

Objects that are hashed, signed, or content-addressed MUST use the deterministic encoding profile required by DARI. Implementations MUST reject non-canonical encodings when an object's hash/signature semantics depend on canonical bytes.
...
DARI opaque identifiers are 128-bit values unless a schema specifies otherwise.
...
- supported DARI core major/minor revisions;
- peer profile;
...
# 16. DARI Peer Credentials

A **DARI Peer Credential (PPC)** is a CP or organization-authority-signed credential binding a peer ID and profile to a public key.
...
DARI's baseline enrollment is software-only. If a user fully controls the host and can extract or reuse the Harness private key, the protocol cannot mathematically prove that the executable presenting that key is the original unmodified binary.

DARI therefore guarantees **enrolled peer identity and protocol authorization**, not remote binary attestation, unless an optional hardware-attestation extension is deployed.
...
Harness MUST also validate Relay's transport certificate and DARI peer credential. A generic TLS endpoint possessing an unrelated Web PKI certificate is not sufficient if enterprise policy requires organization-managed Relay identity.
...
Government deployments MAY use local PKI, SSO, smart-card, or air-gapped identity systems without changing DARI wire semantics.
...
Supporting `dari.voice/1` does not mean the current user is authorized to use voice.
...
A Governed Exchange is DARI's central application primitive.
...
The `lane_id` remains present in DARI records even when QUIC already identifies the transport stream because:
...
A QUIC stream may carry only one DARI lane unless an extension explicitly defines aggregation.
...
The 32-byte DARI prelude identifies the lane and sequence.
...
DARI content-addressed objects use an algorithm selected by the active cryptographic profile.
...
DARI does not create its own global transparency ledger.

Deployments MAY submit DARI Evidence Receipts or selected provenance statements to external transparency infrastructure.
...
DARI conformance does not require a public transparency service, which preserves air-gapped operation.
...
If enabled, the implementation SHOULD use MLS rather than a DARI-defined group key protocol.
...
- COSE;
- ECDSA P-256/SHA-256 for DARI credential/evidence signatures;
...
DARI's registries allow hybrid TLS key exchange and post-quantum signatures where standardized and deployment-approved.

Post-quantum support is optional in DARI v1 and MUST NOT be advertised unless interoperable test vectors pass.
...
# 39. AI Inference Extension (`dari.ai/1`)
...
7. opens downstream DARI inference lane to PIA.
...
PIA authenticates to Relay exactly as a first-class DARI peer.
...
PIA MAY translate DARI to a serving engine's local native or compatibility API internally. That translation is not a DARI network binding and MUST NOT be exposed to Harnesses as an alternative route.

---

# 41. Context Extension (`dari.context/1`)
...
Secrets/PII/injection systems are implementation components; DARI carries their structured decisions.

---

# 42. Tool and Runtime Extension (`dari.tools/1`)
...
DARI integrates with Git/SCM but does not replace it.
...
DARI allows attribution states such as:
...
# 44. Chat Extension (`dari.chat/1`)

DARI collaboration is not an ungoverned side channel.
...
# 46. Voice Extension (`dari.voice/1`)
...
# 47. File Transfer Extension (`dari.file/1`)

DARI-managed file transfer is resumable and auditable.
...
# 48. Broadcast Extension (`dari.broadcast/1`)
...
Transport switch QUIC→TCP may resume the same Working Session after full DARI authentication.
...
DARI MUST NOT silently re-run a completed tool side effect while reconstructing an inference conversation.
...
DARI application priority complements transport flow control; it does not override QUIC congestion control.
...
DARI provides structured visibility; it does not mandate that every administrator may read every payload.
...
DARI remains language-neutral on the wire, while the product profile supports Korean enterprise requirements.
...
DARI does not hard-code a Korean PII detector; it standardizes the decision/result metadata emitted by such detectors.
...
Government Sovereign deployments use the same DARI protocol.
...
DARI implementations SHOULD export internal metrics through deployment-standard observability systems.
...
Until formal standards governance exists, the open-source DARI project maintains public registries for:
...
DARI is open. Reverse engineering is not a threat to protocol confidentiality because no security property depends on secrecy of message definitions.

## 73.2 Modified Harness

Without hardware-rooted attestation, an attacker controlling the machine may modify the Harness and potentially reuse its enrolled key. DARI cannot provide impossible binary-integrity guarantees under that threat.
...
A malicious user can write another HTTPS program unless endpoint/network controls prevent it. Strong enterprise "no external model" policy requires operating-system/network egress controls in addition to DARI.
...
Wall-clock time is useful for records but is not a sole security ordering primitive. DARI relies on nonces, sequence numbers, policy/lease validity, causal hashes, and signed receipts.

---

# 74. Privacy Considerations

DARI can expose unusually rich enterprise activity data. Implementers must avoid assuming that administrator role implies unlimited content access.
...
These systems solve adjacent problems. DARI does not require reimplementation of them.

Example:

```text
DARI Harness session
  ├─ model inference via DARI
  ├─ tool may internally speak MCP
  ├─ PIA workload may receive SPIFFE identity
  ├─ commit provenance exported to in-toto/SLSA
  └─ DARI Evidence Receipt optionally registered in SCITT
```
...
```text
Harness -- QUIC DARI --> Relay
       network path blocks UDP
       connection lost

Harness -- TLS/TCP DARI --> Relay
       HELLO/AUTH again
...
- ALPN protocol identifier for DARI;
- media types for exported DARI evidence objects if required;
...
This document does not claim an existing IANA allocation for `dari/1`.

---

# 86. Standards Baseline

DARI intentionally builds on established standards rather than replacing them.
...
Standards references define cryptographic/encoding primitives; DARI defines the application-level authority and provenance semantics.

---

# 87. Definition of Protocol Completion

DARI v1 is implementation-ready when all of the following are true:
...
DARI does not require a permanent fork of a serving engine.

Preferred integration:

```text
Relay <--DARI--> PIA <--local adapter--> serving engine
```
...
cose-protected-dari = {
  1 => -8,                             ; EdDSA
  4 => bstr .size (1..64)             ; key identifier
}

cose-key-ed25519 = {
  1 => 1,                              ; OKP
  -1 => 6,                             ; Ed25519
  -2 => bstr .size 32                 ; public key
}
```
...
```cddl
peer-role = 1 / 2 / 3 / 4              ; RELAY / INFERENCE / EFFECT / VERIFIER
...
peer-credential-body = {
  ...
  14 => peer-role,
  15 => [1*16 identifier],           ; exact protocol profiles authorized
  ? 254 => critical-fields,
  ? 255 => extensions
}
...
auth-proof-body = {
  ...
  10 => peer-role                     ; authenticated peer role
}
...
auth-challenge-body = {
  1 => dari-version,
  2 => identifier,                   ; challenge ID
  3 => nonce32,                       ; server nonce
  4 => digest32,                      ; transcript hash
  5 => [1*16 identifier],             ; offered profiles
  6 => bstr .size (1..255),           ; channel binding
  ? 254 => critical-fields,
  ? 255 => extensions
}
```

`auth-challenge-body` is the canonical payload of `AUTH_CHALLENGE` (`0x0101`) and `auth-proof` is the canonical attached-COSE payload of `AUTH_PROOF` (`0x0102`) on the control stream. Let `hello` and `hello_ack` be the exact canonical DARI payload bytes accepted by each peer, excluding record headers. The transcript hash is `SHA-256("DARI-TRANSCRIPT-v1\0" || uint32_be(len(hello)) || hello || uint32_be(len(hello_ack)) || hello_ack)` and label `4` of `auth-challenge-body` MUST equal it. The channel binding is `SHA-256("DARI-CHANNEL-BINDING-v1\0" || exporter_value)`, where `exporter_value` is the TLS 1.3 exporter for native transports or the WebTransport/HTTP-3 exporter defined by `dari.web/1`. The negotiated profile set MUST be sorted by encoded byte order with no duplicates. The `auth-proof` external AAD is `DARI-AUTH-PROOF-v1\0`; it MUST be signed by the subject key, and its protected `kid` MUST equal the subject-key thumbprint. The explicit nonce, role, profile, challenge, and channel-binding fields MUST equal the values authenticated by the transcript and connection. A verification API MUST receive this complete negotiated authentication context; an opaque transcript parameter is conforming only if it contains every value above without permitting caller substitution. Proof of possession is required only for the peer presenting the credential; parent grants are validated by their issuer signatures and credential/revocation state, not by an unavailable historical transport transcript.

A verifier MUST, in order: validate canonical encodings and critical fields; validate protected headers and require body/payload equality; resolve and validate the issuer authority and trust domain; validate the issuer signature; validate time; validate current revocation state from a fresh Signed State Checkpoint; validate the authenticated role against label `14`, the negotiated profile set against label `15`, and the protocol-version set against label `11`; validate the transcript signature and subject-key thumbprint; and only then bind the authenticated peer ID to the connection. Failure at any step is `AUTHENTICATION_FAILED`; an implementation MAY expose a more specific code only to an authorized diagnostic principal.
...
An exchange state is `OPEN`, `AUTHORIZED`, `RUNNING`, `FINALIZING`, `COMPLETED`, `DENIED`, `ABORTED`, or `EVIDENCE_FAILURE`. A terminal state MUST NOT transition. `RUNNING` may enter `FINALIZING` only after the protected operation has stopped producing new effects and all post-action obligations are being evaluated. `FINALIZING` may enter `COMPLETED` only after every required obligation is satisfied, every transactional effect is terminal, all required evidence is durably committed, the Evidence Receipt body is finalized, and every required Receipt Attestation is present and valid; it enters `EVIDENCE_FAILURE` on any finalization failure. A protected operation MUST NOT be forwarded before the exchange reaches `AUTHORIZED`.
...
obligation-update = {
  1 => dari-version,
  2 => digest32,                     ; decision digest
  3 => identifier,                   ; obligation ID
  4 => obligation-state,             ; new state
  5 => identifier,                   ; responsible actor
  6 => digest32,                     ; satisfaction/failure evidence
  7 => uint64,                       ; monotonic update sequence
  8 => time-ms,
  ? 254 => critical-fields,
  ? 255 => extensions
}
...
A signed `obligation-update` is the canonical payload of `APPROVAL_RESULT` (`0x0305`) and uses external AAD `DARI-OBLIGATION-UPDATE-v1\0`. The update sequence is strictly increasing per `(decision digest, obligation ID)` and is committed as an evidence event. The actor MUST equal the responsible peer, and the evidence digest MUST resolve to an event that proves the transition. A receiver MUST reject an update that skips a state, changes a terminal state, repeats a sequence with a different body, or satisfies an obligation without the required evidence.
...
state-content-ref = {
  ...
  ? 5 => bstr                        ; inline canonical content bytes
}

signed-state-checkpoint = cose-sign1
```

The Signed State Checkpoint external AAD is `DARI-SIGNED-STATE-CHECKPOINT-v1\0`; its digest is `signed_object_digest(0x0305, signed-state-checkpoint)`. The signer MUST be authorized for the state class and trust domain. `expires_at` MUST be greater than `issued_at`, and `maximum staleness` MUST be nonzero and within the implementation's configured upper bound. The content resolver obtains canonical CBOR bytes by the content digest, optionally using label `4`, and MUST verify `SHA-256("DARI-STATE-CONTENT-v1\0" || deterministic_cbor([content type, content kind, content bytes]))` equals label `3` before using the state. If label `5` is present, it is the complete canonical state-content body and MUST hash to label `3`; if it is absent, the resolver MUST retrieve the bytes by label `3` from the authenticated state service before the checkpoint can be used. The registered content type MUST select a published schema for the state class, and the resolved body MUST carry the checkpoint sequence and state class so a digest cannot be rebound to a different stream. A delta MUST identify its base inside the registered content schema, and that base MUST equal the preceding checkpoint's resolved state digest.
...
A fork, missing predecessor, or reset not authorized by an out-of-band trust-baseline operation is `STATE_ROLLBACK`. Sequence/predecessor validation and the high-water update MUST be one durable atomic compare-and-set so concurrent children of one predecessor cannot both succeed. Rollback MUST fail closed for every freshness class. A checkpoint MAY be delivered inline as a signed object in the `checkpoint` field of `RELAY_VERDICT`, `LEASE_RENEW`, `MODEL_READY`, `ENDPOINT_LEASE_STATUS`, or `EVIDENCE_CHECKPOINT`, or resolved by its signed-object digest through the configured state service. The carrier MUST declare which state class it carries, and a profile MUST NOT silently reinterpret a checkpoint from another class; transport does not change these validation rules and Phase 2 allocates no standalone checkpoint message.
...
attestation-claim-class = 1 / 2 / 3 / 4 / 5
; decision/state, inference input/output, effect, evidence root, provenance

attestation-claim = {
  1 => attestation-claim-class,
...
The Evidence Receipt body digest is `object_digest(0x0302, evidence-receipt-body)`. The Receipt Attestation external AAD is `DARI-RECEIPT-ATTESTATION-v1\0`; its digest is `signed_object_digest(0x0703, receipt-attestation)`. Receipt label `22` MUST be the sorted, duplicate-free set derived from committed event classes: Governance Relay (`1`) is required for every non-empty exchange; Inference Peer (`2`) is required when an inference input or output event exists; Effect Executor (`3`) is required when label `16` is non-empty. A producer MUST NOT self-declare a role in label `22` without the corresponding event class. The Evidence Receipt is complete only when its body is canonical and every derived role has at least one valid, in-scope attestation. The receipt MUST satisfy `last_sequence - first_sequence + 1 = event_count`, without integer overflow, and `event_count` MUST be nonzero.
...
The `dari/1` baseline segment size is 1024 events. Another power of two from 16 through 65536 MAY be negotiated before the exchange opens and MUST be bound into its extension map. It MUST NOT change within an exchange. For `N = event_count`, `S = segment_size`, and `N > 0`, `segment_count = max(1, ceil(N / S))`; segment `s` covers sequence positions `[s*S, min((s+1)*S, N))`, and its `actual_count` is the length of that half-open range.
...
If label `7` is present, it MUST contain the canonical `omission-manifest` bytes and its digest MUST equal `SHA-256("DARI-OMISSION-MANIFEST-v1\0" || label_7_bytes)` and receipt label `20`; if receipt label `20` is present, label `7` is required when verifying disclosure completeness. Omitted ranges use half-open `[start,end)` sequence bounds and MUST be ordered, non-overlapping, within receipt sequence bounds, and disjoint from disclosed events. An omission manifest states only that a committed range was withheld for the named policy reason. It MUST NOT state or imply that undisclosed content did not exist, was harmless, or was observed by a party that did not attest it. A selective proof proves inclusion and receipt consistency, not the truth of undisclosed content.
...
Effect Prepare, Effect Authorization, Effect Result, and a status response each use an attached COSE signature. Their external AAD values are respectively `DARI-EFFECT-PREPARE-v1\0`, `DARI-EFFECT-AUTHORIZATION-v1\0`, `DARI-EFFECT-RESULT-v1\0`, and `DARI-EFFECT-STATUS-v1\0`. Their signed-object types are `0x0610`, `0x0611`, `0x0612`, and `0x0613`. A status request has label `8 = 1` and MUST omit labels `4` through `7`; it MAY be authenticated by its enclosing DARI connection. A status response has label `8 = 2`, MUST include the current state and every available binding digest, and MUST be signed by the executor. A receiver MUST reject a response-shaped request, a request-shaped signed response, or a response whose operation/nonce does not match the request.
...
The following negotiation and profile bodies are normative. They are carried in `CAPABILITIES` (`0x0003`) and the profile-specific message families listed in Section 76; generated types or documentation do not replace these runtime contracts:

```cddl
profile-status = 1 / 2 / 3             ; EXACT / DEGRADED / UNSUPPORTED
capability-offer = {
  1 => identifier,                      ; capability ID
  2 => uint8                           ; critical: 0 or 1
}
profile-offer = {
  1 => identifier,                      ; profile ID
  2 => [1*64 capability-offer]
}
profile-result = {
  1 => identifier,                      ; profile ID
  2 => profile-status,
  ? 3 => [0*64 identifier],             ; omitted capabilities
  ? 4 => identifier,                    ; reason code
}
profile-negotiation = {
  1 => dari-version,
  2 => [1*32 profile-offer],
  3 => [1*32 profile-result],
  ? 254 => critical-fields,
  ? 255 => extensions
}
...
inference-request-body = {
  1 => dari-version,
  2 => identifier,                      ; exchange ID
  3 => digest32,                        ; model manifest digest
  4 => digest32,                        ; endpoint authorization digest
  5 => digest32,                        ; canonical input digest
  6 => identifier,                      ; response mode
  ? 7 => uint64,                        ; max output units
  ? 254 => critical-fields,
  ? 255 => extensions
}
model-artifact-manifest-body = {
  1 => dari-version,
  2 => identifier,                      ; artifact ID
  3 => digest32,                        ; artifact digest
  4 => identifier,                      ; format
  5 => [1*16 identifier],               ; supported runtimes
  6 => identifier,                      ; issuer
  7 => time-ms,
  8 => time-ms,
  ? 254 => critical-fields,
...
endpoint-authorization-body = {
  1 => dari-version,
  2 => identifier,                      ; endpoint ID
  3 => identifier,                      ; model artifact ID
  4 => identifier,                      ; serving peer ID
  5 => [1*16 identifier],               ; allowed profiles
  6 => time-ms,
  7 => time-ms,
  ? 254 => critical-fields,
  ? 255 => extensions
...
web-binding-body = {
  1 => dari-version,
  2 => uri,                             ; origin
  3 => digest32,                        ; browser proof transcript
  4 => identifier,                      ; reconnect session ID
  5 => nonce32,                         ; reconnect nonce
  ? 254 => critical-fields,
  ? 255 => extensions
}
federation-trust-bundle-body = {
  1 => dari-version,
  2 => identifier,                      ; trust domain
  3 => [1*64 identifier],               ; issuers
  4 => [1*64 digest32],                 ; issuer key digests
  5 => [1*64 identifier],               ; audiences
  6 => uint64,                          ; bundle sequence
  7 => time-ms,
  8 => time-ms,
  ? 254 => critical-fields,
  ? 255 => extensions
}
collab-envelope-body = {
  1 => dari-version,
  2 => identifier,                      ; conversation/channel ID
  3 => uint64,                          ; ordered message sequence
  4 => identifier,                      ; sender peer ID
  5 => digest32,                        ; ciphertext digest
  6 => time-ms,
  ? 254 => critical-fields,
  ? 255 => extensions
}
media-chunk-body = {
  1 => dari-version,
  2 => identifier,                      ; media session ID
  3 => uint64,                          ; chunk sequence
  4 => digest32,                        ; previous chunk digest
  5 => digest32,                        ; encrypted chunk digest
  6 => uint32,                          ; duration milliseconds
  ? 254 => critical-fields,
  ? 255 => extensions
}
file-transfer-body = {
  1 => dari-version,
  2 => identifier,                      ; transfer ID
  3 => uint64,                          ; chunk sequence
  4 => uint64,                          ; total chunks
  5 => digest32,                        ; chunk digest
  6 => digest32,                        ; whole-file digest
  7 => uint32,                          ; byte count
  ? 254 => critical-fields,
  ? 255 => extensions
}
...
Capability and profile arrays MUST be sorted by deterministic encoding and contain no duplicates. The responder MUST return exactly one result for every offer, and a result MUST NOT name an unoffered profile. A critical capability omitted from `DEGRADED` is a negotiation failure. Each profile body is bound to the exchange, grant, decision, checkpoint, and receipt rules of `dari/1`; a profile adapter MUST report `DEGRADED` only when the omitted capability is explicitly non-critical and its loss is recorded in evidence.
...
A conformance suite MUST exercise at least these cases and observe no new or unauthorized protected side effect on rejection. If a previously committed external effect is encountered after a later failure, the test MUST observe durable status reconciliation and an evidence failure rather than a fabricated rollback or duplicate execution:
